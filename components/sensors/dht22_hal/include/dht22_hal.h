...
typedef enum : uint8_t {
  ...
  k_dht22_init_error      = 0xA1, /**< Error during initialization. */
  k_dht22_read_error      = 0xA2, /**< Error during data read. */
  k_dht22_checksum_error  = 0xA3, /**< Data checksum verification failed. */
  k_dht22_timeout_error   = 0xA4, /**< Sensor response timeout. */
} dht22_states_t;
 ...
 * Error handling and retry logic is managed by the error handler component.
 */
typedef struct {
 ...
 * `dht22_data_t` structure to store sensor readings. Also initializes the error
 * handler for managing error recovery.
 *
...
/**
 * @brief Handles reinitialization and error recovery for the DHT22 sensor.
 *
 * Triggers the error handler to attempt sensor recovery when errors are detected.
 * The error handler manages retry timing and exponential backoff.
 *
 ...
 *
 * Periodically reads temperature and humidity data from the sensor and transmits
 * data to a web server. Error recovery is handled by the error handler component.
 *
 * @param[in,out] sensor_data Pointer to the `dht22_data_t` structure managing
 *                            sensor data.
 */
